...
export const listSignNowTemplates = async (req, res) => {
    try {
        ...
        const templates = rawTemplates.map(tmpl => ({
            ...
            name: tmpl.document_name || tmpl.name || "Untitled Agreement Template",
...
export const createSignNowTemplate = async (req, res) => {
    try {
        ...
        // STEP 2: Flatten the base blueprint document asset into a permanent Workspace Template model
        ...
        // STEP 3: AUTOMATED PIPELINE CLEANUP - Instantly delete the base document wrapper asset container
        try {
        ...
        // STEP 4: FIX - Pass explicit field configuration preferences to initialize the full premium dashboard layout fields panel
        const sessionApiResponse = await axios.post(
            `https://api.signnow.com/v2/documents/${templateId}/embedded-sending`,
            {
                type: "document",
                link_expiration: 45,
        ...
        return res.status(200).json({
            ...
            template: { id: templateId, name, description: description || "Generated via Express CRM Asset Manager" },
...
export const getTemplateEditingSession = async (req, res) => {
    try {
        ...
        // FIX: Synchronized field panel preference attributes to ensure existing templates open with all widgets unlocked
        const apiResponse = await axios.post(
            `https://api.signnow.com/v2/documents/${template_id}/embedded-sending`,
            {
                type: "document",
                link_expiration: 45,
                redirect_uri: "https://signnow.com",
